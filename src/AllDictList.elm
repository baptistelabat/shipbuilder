-- https://github.com/Gizra/elm-dictlist/tree/2.1.2/src
...
type alias AllDict k v comparable =
    Dict.Any.AnyDict comparable k v
...
decodeObject decoder =
    Json18.map (fromList identity) (keyValuePairs decoder)
        ...
        go jsonValue key accum =
            case ( accum, decodeValue (func key) jsonValue ) of
                ...
                ( Err err1, Err err2 ) ->
                    ...
                    Err <| Json18.OneOf [ err1, err2 ]
    in
    value
        |> Json18.andThen
            (\jsonValue ->
                case List.foldl (go jsonValue) (Ok (empty ord)) keys_ of
                    Ok result ->
                        Json18.succeed result

                    Err err ->
                        Json18.fail <| Json18.errorToString err
            )
...
decodeKeysAndValues ord keyDecoder func =
    keyDecoder
        |> Json18.andThen (\keys_ -> decodeWithKeys ord keys_ func)
...
decodeArray2 ord keyDecoder valueDecoder =
    Json18.map2 Tuple.pair keyDecoder valueDecoder
        |> Json18.list
        |> Json18.map (fromList ord)
...
cons key value (AllDictList dict list) =
    AllDictList
        (Dict.Any.insert key value dict)
...
head (AllDictList dict list) =
    List.head list
        |> Maybe18.andThen (\key -> Dict.Any.get key dict |> Maybe.map (\value -> ( key, value )))
...
tail (AllDictList dict list) =
    case list of
        first :: rest ->
            Just <|
                AllDictList (Dict.Any.remove first dict) rest
...
--             ( index + 1
--             , AllDictList
--                 (Dict.Any.insert key (func index key value) dict)
--                 (key :: list)
--             )
--     in
...
maximum (AllDictList dict list) =
    ...
    in
    Dict.Any.foldl go Nothing dict
...
--
--         newDict =
--             List.foldl go (Dict.Any.empty (Dict.Any.getOrd dict)) newList
--
--         go key =
--             Dict.Any.insert key (unsafeGet key dict)
--     in
...
insertAfter afterKey key value (AllDictList dict list) =
    let
        ...
        newList =
            if Dict.Any.member afterKey dict then
...
insertBefore beforeKey key value (AllDictList dict list) =
    let
        ...
        newList =
            if Dict.Any.member beforeKey dict then
...
empty ord =
    AllDictList (Dict.Any.empty ord) []
...
insert key value (AllDictList dict list) =
    let
        ...
        newList =
            if Dict.Any.member key dict then
...
singleton ord key value =
    AllDictList (Dict.Any.singleton key value ord) [ key ]
...
--             case Dict.Any.get rightKey remainingRight of
--                 Just rightValue ->
...
map func (AllDictList dict list) =
    AllDictList (Dict.Any.map func dict) list
...
-}
foldl : v -> (k -> v -> b -> b) -> b -> AllDictList k v comparable -> b
foldl default func accum (AllDictList dict list) =
    let
        go key acc =
            func key (unsafeGet default key dict) acc
    in
...
-}
filter : v -> (k -> v -> Bool) -> AllDictList k v comparable -> AllDictList k v comparable
filter default predicate dictList =
    let
        add key value dict =
            if predicate key value then
                ...
                dict
    in
    foldl default add (emptyWithOrdFrom dictList) dictList
...
fromAllDict dict =
    AllDictList dict (Dict.Any.keys dict)
...
fromDict dict =
    let
        allDict =
            Dict.foldl Dict.Any.insert (Dict.Any.empty identity) dict
    in
...
-}
unsafeGet : v -> k -> AllDict k v comparable -> v
unsafeGet default key dict =
    case Dict.Any.get key dict of
        Just value ->
            value
...
-}
equalKeys : k -> k -> AllDict k v comparable -> Bool
equalKeys key1 key2 dict =
    let
        ord =
            Dict.Any.getOrd dict
    in
