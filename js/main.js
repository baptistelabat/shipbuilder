...
app.ports.toJs.subscribe(function (message) {
  ...
  switch (message.tag) {
    ...
    case "read-json-file-open":
      // used to read a save file
      readFile("save-data", data);
      break;

    case "read-json-file-import":
      // used to read a save file
...
var setObjectOpacityForCurrentMode = function setObjectOpacityForCurrentMode(object) {
  if (object.sbType === mode || object.sbType == "exception") {
...
var readFile = function readFile(cmd, inputId) {
...
var readClipboard = function readClipboard(cmd, inputId) {
...
var makeBulkheads = function makeBulkheads(bulkheads) {
  ...
  var bulkheadColor = new THREE.Color(0.5, 0.5, 0.5); // grey
...
var showCenterOfGravity = function showCenterOfGravity(cogCoordinates) {
  //Delete old center of gravity
  var oldCentersOfGravity = scene.children.filter(function (child) {
    return child.sbType // checks if the sbType attribute exists to prevent JS from crashing on an undefined check
    && child.sbType === "exception";
  });
  oldCentersOfGravity.forEach(function (oldCenterOfGravity) {
    return removeFromScene(oldCenterOfGravity);
  }); //Create centers of gravity to be displayed on top, side and front of the model

  makeLandMark("exception", 1, {
    x: 250,
    z: -cogCoordinates.z,
    y: cogCoordinates.y
  }, {
...
var makeLandMark = function makeLandMark(type, radius, position, rotation) {
  var color = new THREE.Color(0.5, 0.5, 0.5); // grey
  ...
  circle.rotation.set(rotation.x, rotation.y, rotation.z); //Create two lines which cross in the circle

  var lineGeometryH = new THREE.Geometry();
  var lineGeometryV = new THREE.Geometry(); //horizontal line
  ...
  lineH.rotation.set(rotation.x, rotation.y, rotation.z); //vertical line
  ...
  lineV.rotation.set(rotation.x, rotation.y, rotation.z); //Add landMark to the scene
...
var makeDecks = function makeDecks(decks) {
  ...
  var deckColor = new THREE.Color(0.5, 0.5, 0.5); // grey
  // color of the deck #0
...
var deleteWaterline = function deleteWaterline() {
  var oldWaterLines = scene.children.filter(function (child) {
    return child.sbType && child.sbType === "modeller" && child.modellerType && child.modellerType === "waterLine";
  });
  oldWaterLines.forEach(function (oldWaterLine) {
    return removeFromScene(oldWaterLine);
  });
};

var addSquareVertices = function addSquareVertices(geometry, size) {
  geometry.vertices.push(toThreeJsCoordinates(-size / 2, -size / 2, 0, coordinatesTransform));
  geometry.vertices.push(toThreeJsCoordinates(-size / 2, size / 2, 0, coordinatesTransform));
  geometry.vertices.push(toThreeJsCoordinates(size / 2, size / 2, 0, coordinatesTransform));
  geometry.vertices.push(toThreeJsCoordinates(size / 2, -size / 2, 0, coordinatesTransform));
...
var displayWaterLine = function displayWaterLine(zPosition) {
  deleteWaterline();
  var blue = new THREE.Color(0.5, 0.5, 1);
  var size = 500;
...
var addVerticesForSlice = function addVerticesForSlice(json, slice, ny, geometry) {
...
var addFacesForSlices = function addFacesForSlices(nx, ny, geometry) {
...
var closeMeshForHull = function closeMeshForHull(nx, ny, geometry) {
  var i = 0;
...
var loadHull = function loadHull(json) {
  ...
  if (hullSlices.slices.length > 0) {
    ...
    var geometry = buildHullGeometry(hullSlices);
    var volume = calcVolume(geometry);
    var blue = new THREE.Color(3 / 255, 146 / 255, 255 / 255);
...
var deleteHighlightedSlice = function deleteHighlightedSlice() {
  var oldHighlights = scene.children.filter(function (child) {
    return child.sbType && child.sbType === "modeller" && child.modellerType && child.modellerType === "highlightSlice";
  });
  oldHighlights.forEach(function (oldHighlight) {
    return removeFromScene(oldHighlight);
  });
};

var buildSliceGeometry = function buildSliceGeometry(slice, depth, breadth, length, xmin, zmin) {
  var highlightMarginWithHull = 0.005;
  var ymin = -breadth / 2;
  var x = slice['x'];
...
var displayHighlightedSlice = function displayHighlightedSlice(slice, depth, breadth, length, xmin, zmin) {
  var geometry = buildSliceGeometry(slice, depth, breadth, length, xmin, zmin);
  var colorRed = new THREE.Color("hsl(11, 80%, 60%)");
  var material = new THREE.LineBasicMaterial({
    color: colorRed,
    linewidth: 1,
    side: THREE.DoubleSide
  });
  var sliceToConstruct = new THREE.LineLoop(geometry, material);
...
var resetBlockSelection = function resetBlockSelection() {
  if (selection.length) {
