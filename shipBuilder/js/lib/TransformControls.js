 ...
 * @author colleb // edited to work with multiple viewports
 */

THREE.TransformControls = function (view, domElement) {
  ...
  defineProperty("camera", view.camera);
  defineProperty("view", view);
  ...
  function getViewportBounds(view) {
    ...
    var topLimit = rect.top + view.top * domElement.height;
    var leftLimit = rect.left + view.left * domElement.width;
    var rightLimit = leftLimit + view.width * domElement.width;
    var bottomLimit = topLimit + view.height * domElement.height;
    return {
  ...
  function normalizeMouseCoordinates(mouse) {
    var viewBoundingBox = getViewportBounds(view);
    var normalized = new THREE.Vector2();
    var boxWidth = viewBoundingBox.bottomRight.x - viewBoundingBox.topLeft.x;
    var boxHeight = viewBoundingBox.bottomRight.y - viewBoundingBox.topLeft.y;
    var mouseOffset = new THREE.Vector2(mouse.x - viewBoundingBox.topLeft.x, mouse.y - viewBoundingBox.bottomRight.y);
  ...
  function getPointer(event) {
    ...
    var mouse = new THREE.Vector2(pointer.clientX, pointer.clientY);
    var normalizedForView = normalizeMouseCoordinates(mouse);
    return {
      x: normalizedForView.x,
      y: normalizedForView.y,
      button: event.button
    };
...
THREE.TransformControlsGizmo = function () {
  ...
  this.updateMatrixWorld = function () {
    ...
    for (var i = 0; i < handles.length; i++) {
      ...
      handle.position.copy(this.worldPosition); //var eyeDistance = this.worldPosition.distanceTo(this.cameraPosition);
      //handle.scale.set(1, 1, 1).multiplyScalar(eyeDistance * this.size / 7);

      handle.scale.set(1, 1, 1).multiplyScalar(this.size); // TODO: simplify helpers and consider decoupling from gizmo
